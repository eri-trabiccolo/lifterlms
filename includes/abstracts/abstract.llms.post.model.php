...
abstract class LLMS_Post_Model implements JsonSerializable {
	 ...
	 * @since [version] Introduced new param.
	 *
	 ...
	 * @param boolean      $allow_same_meta_value Optional. Whether or not updating a meta with the same value as stored in the db is allowed. Default is `false`.
	...
	public function set_bulk( $model_array, $wp_error = false, $allow_same_meta_value = false ) {
		...
		if ( is_wp_error( $update_post_properties ) ) {
			$error = $update_post_properties;
		}
	 ...
	 * @since [version]
	 *
	 * @param [type] $model_array
	 * @return array|false False if nothing to set. An array that contains all the post properties and all the metas to set.
	 */
	...
	private function update_meta_properties( $post_meta_properties, $allow_same_meta_value ) {
		...
		foreach ( $post_meta_properties as $key => $val ) {
			...
			if ( $allow_same_meta_value ) {
				// Do the exact same check for a duplicate value as in update_metadata() to avoid update_post_metad() returning false.
				// see WP_REST_Meta_Fields::update_meta_value().
				$old_value = get_post_meta( $this->id, $this->meta_prefix . $key, false );
				if ( 1 === count( $old_value ) && $this->is_meta_value_same_as_stored_value( $key, $old_value[0], $val ) ) {
					return;
				}
			}
	 ...
	 *
	 * @param string $key          The unprefixed meta key being checked.
	...
	private function is_meta_value_same_as_stored_value( $key, $stored_value, $new_value ) {
		...
		if ( in_array( $this->get_property_type( $key ), array( 'string', 'text', 'absint', 'yesno', 'html', 'float' ), true ) ) {
